...
function checkQR() {
  /**
  * Checks a QR code guess by hashing the username and dogifying the sum.
  */
  return function(guess, username) {
    var crypto = require('crypto');
    var SECRET = "d09e4l143";
    var DOGEWORDS = ['much', 'such', 'wow', 'many', 'very', 'amaze'];
    ...
    sum.update(username + SECRET);
    var hexCode = sum.digest('hex');

    var hexSum = 0;
    for (var i in hexCode) hexSum += hexCode.charCodeAt(i);
    var answer = DOGEWORDS[hexSum % DOGEWORDS.length] + " " + NOUNS[hexSum % NOUNS.length];
